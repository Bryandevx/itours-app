<template>
  <v-card class="mx-auto" width="315" height="">
    <v-img class="white--text align-end" height="150px" :src="tour.promo_img">
    </v-img>

    <v-card-title>{{ tour.name }}</v-card-title>

    <v-card-subtitle class="pb-0"> 0 reviews</v-card-subtitle>
...
export default {
  ...
  methods: {
    ...
    loadPromoImg() {
      let result = this.promoImgs.find(
        (element) => element.tour_id === this.tour.id
      );
      console.log("RESULT FROM CARD LIKED");
      console.log(result);
